...
func (_Flow *FlowFilterer) FilterSubmission(opts *bind.FilterOpts, sender []common.Address, identity [][32]byte) (*FlowSubmissionIterator, error) {
	...
	logs, sub, err := _Flow.contract.FilterLogs(opts, "Submission", senderRule, identityRule)
	if err != nil {
		return nil, err
	}
	return &FlowSubmissionIterator{contract: _Flow.contract, event: "Submission", logs: logs, sub: sub}, nil
}
...
func (_Flow *FlowFilterer) WatchSubmission(opts *bind.WatchOpts, sink chan<- *FlowSubmission, sender []common.Address, identity [][32]byte) (event.Subscription, error) {
	...
	logs, sub, err := _Flow.contract.WatchLogs(opts, "Submission", senderRule, identityRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(FlowSubmission)
				if err := _Flow.contract.UnpackLog(event, "Submission", log); err != nil {
					return err
				}
				event.Raw = log
